...
{
    public class ControllerListaBase<TAplic, TView> : ControllerCoreBase<TAplic>
        where TView : IdView, new()
        where TAplic : IAplicListaBase<TView>
    {
        public ControllerListaBase(TAplic aplic) : base(aplic)
        {
        }
        ...
        [Route("{id}")]
        [HttpGet]
        public virtual IActionResult Listar([FromRoute] int id, [FromQuery] QueryField dto)
        {
            try
            {
